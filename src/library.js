...
export default {
    ...
    // 储存当前用户的台账及技术评标的搜索数据 (只储存3个，当超出时，去掉第一个)
    setlocalStorageUserData({
        ...
        /**
         * @param userCode {string} 用户code 在登录时储存到localStorage里
         * @param requestKey {string} 要储存的数据key值
         * @param requestData {object} 要储存的数据value值
         */
        ...
        if (userData.length) {
            for (let i = 0, l = userData.length; i < l; i++) {
                let userDataI = userData[i];
                if (userDataI.userCode == Vue.prototype.vm.$store.state.common.userInfo.userlogin) {
                    newUser = userDataI;
                    userData.splice(i, 1);
                    break;
                }
        ...
        //   let aa = localStorage.getItem("userData");
        //   console.log(JSON.parse(localStorage.getItem("userData")),'111111111111')    
    },
